...
def to_client(conn, addr) :
    ...
    try:
        ...
        df_and = data[
            (data['가수'].replace(' ', '') == target_singer) & (data['제목'].replace(' ', '') == target_title)]
    ...
    if flag == 1:
        ...
        data.loc[len(data)] = [target_title,target_singer,target_genre,target_days,target_album,target_lyric,target_season]
        #data = data.append(temp, ignore_index=True)
        df = pd.DataFrame(data)
        if not os.path.exists('가사전처리수정본2.csv'):
            df.to_csv('가사전처리수정본2.csv', index=False, mode='w', encoding='utf-8-sig', header=False)
        else:
            df.to_csv('가사전처리수정본2.csv', index=False, mode='w', encoding='utf-8-sig', header=False)
        data = pd.read_csv('가사전처리수정본2.csv', low_memory=False)
    ...
    if stop == 1:
        ...
        result = get_recommendations(target_title, target_singer, target_genre, target_season)
    else :
