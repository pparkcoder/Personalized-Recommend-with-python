...
def check(url,search_name) : # 식당에서 해당 수산물을 판매하는지 판별
...
if __name__ == "__main__":
    ...
    for k in range(len(search_data)):
        ...
        url = 'https://www.diningcode.com/list.php?query=%EC%84%9C%EC%9A%B8%20' + urllib.parse.quote_plus(
            search_data[k])
        driver.get(url)
        ...
        for i in range(len(result)):
            ...
            total_data = []
            temp = []
            if check(temp_url, search_data[k]) == 1: # 식당에 해당 수산물을 판매하는 
