    ...
    for i in data:
        ...
        if min_len > 0:
            for j in range(1, min_len):
                ...
                temp_album = re.sub('[\{\}\[\]\/?.`!^\-_+<>@\#$%&\\\=\'\"\♥\♡\ㅋ\ㅠ\ㅜ\ㄱ\ㅎ\ㄲ\ㅡ]', '',
                                    album[j].text).lower()
                ...
                input_check = driver.find_element_by_xpath(
                    '//*[@id="content"]/div/div[3]/div[1]/div/table/tbody/tr[' + str(j) + ']').get_attribute('class')
                ...
                if (search_song in temp_song) and (search_artist in temp_artist) and (search2_album in temp2_album):
                    try:
                        driver.find_element_by_xpath('//*[@id="content"]/div/div[3]/div[1]/div/table/tbody/tr[' + str(
                            j) + ']/td[3]/div[1]/span/a').click()
                        driver.implicitly_wait(10)
                        flag = 1  # 동일 곡이 있는 경우
                        try:  # 가사가 있는 경우
                            try:
                                lyric = driver.find_element_by_css_selector('.lyrics.hide')
                                temp_list.append(lyric.text)
                                lyric_data.append(temp_list)
            ...
            if flag == 0:
                for j in range(1, min_len):
                    ...
                    input_check = driver.find_element_by_xpath(
                        '//*[@id="content"]/div/div[3]/div[1]/div/table/tbody/tr[' + str(j) + ']').get_attribute(
                        'class')
                    ...
                    if (search_song in temp_song) and (search_artist in temp_artist):
                        try:
                            driver.find_element_by_xpath(
                                '//*[@id="content"]/div/div[3]/div[1]/div/table/tbody/tr[' + str(
                                    j) + ']/td[3]/div[1]/span/a').click()
                            driver.implicitly_wait(10)
                            try:  # 가사가 있는 경우
                                try:
